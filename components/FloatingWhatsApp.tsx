...
export default function FloatingWhatsApp() {
  ...
  return (
    <aside aria-label="WhatsApp Concierge" className="fixed bottom-5 right-5 sm:bottom-6 sm:right-6 z-50 flex items-center gap-2.5">
      {/* Pill Badge (Exact match to Image 2 screenshot: [🟢 Chat on WhatsApp ✕]) */}
      <AnimatePresence>
        {showPill && !isOpen && (
          <motion.div
            ...
            className="bg-white/95 backdrop-blur-md border border-sand-300/80 px-3.5 py-1.5 sm:py-2 rounded-full shadow-lg shadow-black/5 flex items-center gap-2 text-xs font-medium text-charcoal-900 cursor-pointer hover:bg-sand-50 transition-all select-none"
            onClick={handleDirectWhatsApp}
          >
            <span className="w-2 h-2 rounded-full bg-[#25D366] shrink-0 animate-pulse" />
            <span className="whitespace-nowrap font-medium text-charcoal-900 text-[12px] sm:text-xs">
              Chat on WhatsApp
            </span>
            <button
              onClick={(e) => {
              ...
              className="text-charcoal-400 hover:text-charcoal-700 ml-0.5 p-0.5 rounded-full hover:bg-sand-200/60 transition-colors"
      ...
      {/* Official WhatsApp Circular Button (Exact match to 2nd Image) */}
      <motion.button
        whileHover={{ scale: 1.08 }}
        whileTap={{ scale: 0.94 }}
        onClick={handleDirectWhatsApp}
        className="w-12 h-12 sm:w-13 sm:h-13 rounded-full bg-[#25D366] hover:bg-[#20ba59] text-white flex items-center justify-center shadow-lg shadow-emerald-500/25 relative group shrink-0 transition-colors"
        aria-label="Open WhatsApp Chat"
      >
        <svg
          viewBox="0 0 24 24"
          className="w-7 h-7 sm:w-7.5 sm:h-7.5 fill-current text-white"
