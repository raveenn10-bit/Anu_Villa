...
export default function FloatingWhatsApp() {
  ...
  return (
    <aside aria-label="WhatsApp Concierge" className="fixed bottom-5 right-4 sm:bottom-6 sm:right-6 z-50 flex items-center gap-2.5">
      ...
      <AnimatePresence>
        {showPill && !isOpen && (
          <motion.div
            ...
            className="bg-white/95 backdrop-blur-md border border-sand-300/90 px-4 py-2 sm:px-4 sm:py-2 rounded-full shadow-xl shadow-black/10 flex items-center gap-2 text-xs font-semibold text-charcoal-900 cursor-pointer hover:bg-sand-50 transition-all select-none"
            onClick={handleDirectWhatsApp}
          >
            <span className="w-2.5 h-2.5 rounded-full bg-[#25D366] shrink-0 animate-pulse" />
            <span className="whitespace-nowrap font-bold text-charcoal-950 text-xs sm:text-sm">
              Chat on WhatsApp
            </span>
            <button
              onClick={(e) => {
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute bottom-18 right-0 bg-white rounded-3xl shadow-2xl border border-sand-200 w-80 overflow-hidden"
          >
            {/* Header */}
      ...
      {/* Enlarged WhatsApp Circular Floating Button */}
      <motion.button
        whileHover={{ scale: 1.08 }}
        whileTap={{ scale: 0.92 }}
        onClick={handleDirectWhatsApp}
        className="w-14 h-14 sm:w-14 sm:h-14 rounded-full bg-[#25D366] hover:bg-[#20ba59] text-white flex items-center justify-center shadow-xl shadow-emerald-500/35 border-2 border-white/30 relative group shrink-0 transition-all cursor-pointer"
        aria-label="Open WhatsApp Chat"
      >
        <span className="absolute inset-0 rounded-full bg-[#25D366] animate-ping opacity-20 pointer-events-none" />
        <svg
          viewBox="0 0 24 24"
          className="w-8 h-8 sm:w-8 sm:h-8 fill-current text-white relative z-10"
